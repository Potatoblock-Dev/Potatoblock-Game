/**
 * 你画我猜 DrawingBoard（vendor 自 potatoblock-draw-guess）。
 * Avatar 皮套绘制需要透明底导出，故增加 options.transparent（默认 false）。
 * 笔刷引擎变更时请与 draw_guess/static/js/drawing-board.js 对照同步。
 */
(function (global) {
  'use strict';
  ...
  class DrawingBoard {
    /** 创建画板；transparent=true 时不铺底色，导出保留 Alpha（皮套 UV 用）。 */
    constructor(canvas, options) {
      ...
      this.transparent = Boolean(settings.transparent);
      ...
      canvas.style.backgroundColor = this.transparent ? 'transparent' : this.backgroundColor;
    }
    ...
    /** 设置逻辑背景色（填充取样用）；透明模式下不改变 canvas CSS 底色。 */
    setBackground(color) {
      this.backgroundColor = isHexColor(color) ? color.toLowerCase() : this.defaultBackground;
      this.canvas.style.backgroundColor = this.transparent ? 'transparent' : this.backgroundColor;
    }
    ...
    async exportStrokesBlob(strokes, maxWidth, mimeType, quality) {
      ...
      this.canvas.style.backgroundColor = this.transparent ? 'transparent' : previousBackground;
