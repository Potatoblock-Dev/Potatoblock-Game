/**
 * 多人房间控件：在线状态、人数、创建/加入/返回公共大厅、复制邀请链接。
...
(() => {
  ...
  function bindMultiplayerUi(session) {
    ...
    let lastStatus = session.mode === 'offline' ? 'offline-mode' : 'connecting';
    let everOnline = false;
    /** 展示态：online | connecting | reconnecting | offline | replaced | offline-mode */
    let displayKind = session.mode === 'offline' ? 'offline-mode' : 'connecting';
    let holdTimer = null;
    ...
    function statusText() {
      ...
      if (displayKind === 'offline-mode') return '离线模式';
    ...
    function syncDisplayKind() {
      if (session.mode === 'offline' || lastStatus === 'offline-mode') {
        if (holdTimer) {
          clearTimeout(holdTimer);
          holdTimer = null;
        }
        displayKind = 'offline-mode';
        return;
      }
    ...
    function render() {
      ...
      if (roomEl) {
        roomEl.textContent = session.isPublic || session.roomId === PUBLIC_ROOM_ID
          ? '公共大厅'
          : `房间 ${session.roomId}`;
      }
      if (countEl) {
    ...
    inviteBtn?.addEventListener('click', async () => {
      if (session.isPublic || session.roomId === PUBLIC_ROOM_ID) {
        setFeedback('公共大厅无需邀请链接', true);
        return;
      }
    ...
    session.addEventListener('roomchange', (event) => {
      render();
      const detail = event.detail || {};
      setFeedback(detail.isPublic ? '已进入公共大厅' : `已进入房间 ${detail.roomId}`);
    });
    ...
    session.addEventListener('roomerror', (event) => {
      setFeedback(event.detail?.message || '房间操作失败', true);
      render();
    });

    render();
    return { render, setFeedback };
  }

  window.AvatarMultiplayerUi = { bindMultiplayerUi };
})();
